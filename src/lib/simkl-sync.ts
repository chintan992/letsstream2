 ...
 */
async function fetchFirebaseWatchHistory(userId: string): Promise<WatchHistoryItem[]> {
...
async function importFromSimkl(
    ...
    for (const simklItem of simklItems) {
        try {
            ...
            const existingItem = firebaseHistory.find(item =>
                item.media_id === tmdbId && item.media_type === mediaType
            );
            ...
            const title = simklItem.movie?.title || simklItem.show?.title || simklItem.anime?.title;
            result.errors.push(`Failed to import ${title}: ${error instanceof Error ? error.message : 'Unknown error'}`);
        }
    }
 ...
 */
function shouldSimklOverwrite(simklItem: SimklListItem, firebaseItem: WatchHistoryItem): boolean {
    // For movies: compare completion status
    if (simklItem.movie) {
        // If Firebase has watch position tracking, check if movie was completed
        const firebaseProgress = firebaseItem.watch_position && firebaseItem.duration
    ...
    const firebaseEpisodes = firebaseItem.episodes_watched?.length || 1;
...
async function exportToSimkl(
    ...
    for (const item of firebaseHistory) {
        ...
        if (!simklTmdbIds.has(key)) {
            try {
                await SimklService.checkin(simklToken, {
                    title: item.title,
                ...
                result.errors.push(`Failed to export ${item.title}: ${error instanceof Error ? error.message : 'Unknown error'}`);
            }
        }
    }
...
export async function updateSyncState(
    ...
    try {
        ...
        console.debug("Could not update sync state (missing Firestore rules for 'simklSync')");
    }
}
