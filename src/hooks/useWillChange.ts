...
export const useWillChange = (
  ...
  useEffect(() => {
    ...
    if (animationName && animationDuration) {
      ...
      // Add event listeners
      element.addEventListener('animationend', handleAnimationEnd as EventListener, { passive: true });
      element.addEventListener('transitionend', handleTransitionEnd as EventListener, { passive: true });
  ...
  }, [elementRef, properties, animationName, animationDuration, idleTimeout, cleanupOnUnmount, removeWillChange, setWillChange, getReducedMotion]);

  // Reset idle timeout when needed (for infinite animations)
  const resetIdleTimeout = useCallback(() => {
    if (options.animationName && !options.animationDuration && !getReducedMotion()) {
      setWillChange();
  ...
  }, [options.animationName, options.animationDuration, idleTimeout, getReducedMotion, setWillChange, removeWillChange]);

  // Handle intersection observer for elements that should only have will-change when visible
  const setupVisibilityHandler = useCallback((callback: (isVisible: boolean) => void) => {
    if (getReducedMotion()) return () => {};
  ...
  }, [elementRef, getReducedMotion, options.animationName, options.animationDuration, resetIdleTimeout]);

  // Handle user interaction events to reset idle timeout
  const setupInteractionHandler = useCallback((element: HTMLElement) => {
    if (getReducedMotion() || !options.animationName || options.animationDuration) return () => {};
  ...
  }, [getReducedMotion, options.animationName, options.animationDuration, resetIdleTimeout]);
