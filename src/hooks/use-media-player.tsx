...
export const useMediaPlayer = (
  ...
  useEffect(() => {
    ...
    const sortedSeasons = [...tvDetails.seasons].sort((a, b) => a.season_number - b.season_number);
    const nextSeason = sortedSeasons.find(seasonData => seasonData.season_number > currentSeasonNumber);
    
    if (nextSeason) {
      // Validate that the next season actually has episodes
      const validateNextSeason = async () => {
        try {
          const nextSeasonDetails = await getSeasonDetails(parseInt(id!, 10), nextSeason.season_number);
  ...
  const goToNextEpisode = async () => {
    ...
    try {
      ...
      navigate(`/watch/tv/${id}/${nextSeason.season_number}/${firstEp.episode_number}`);
