...
function simklToMedia(item: SimklTrendingItem, mediaType: "movie" | "tv", index: number): Media & { media_id: number } {
...
const SimklDiscoverList = () => {
    ...
    const media = useMemo(() => {
        if (!config) return [];
        return items.map((item, index) => simklToMedia(item, config.mediaType, index));
    }, [items, config]);
        ...
        return (
            <main className="flex min-h-screen w-full flex-col bg-background">
                ...
                <div className="container mx-auto px-6 pt-24 text-center">
                    ...
                    <Link to="/simkl" className="text-blue-400 hover:underline mt-4 block">
    ...
    const title = category === "recommendations" ? "Recommended For You" : config?.title || "";
    const icon = category === "recommendations"
        ? <Sparkles className="h-6 w-6 text-purple-400" />
        : config?.icon;
    const iconColor = category === "recommendations" ? "text-purple-400" : config?.color;
